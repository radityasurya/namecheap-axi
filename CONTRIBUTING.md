...
transpile — `bin/namecheap-axi.js` runs `src/` directly.

The test suite needs no Namecheap account and makes no network calls. It runs against a stubbed
...
- **Generated files are generated.** `skills/namecheap-axi/SKILL.md` comes from `src/skill.js`
...
`npm view namecheap-axi version`. See [AGENTS.md](AGENTS.md) for the one-time
